...
char *
make_carousel_root(uint16_t elementary_pid, uint32_t carousel_id)
{

	/* make the hierarchy */
	snprintf(_carousel_root, sizeof(_carousel_root), "%s", CAROUSELS_DIR);
	if(mkdir(_carousel_root, 0755) < 0 && errno != EEXIST)
		fatal("Unable to create carousel directory '%s': %s", _carousel_root, strerror(errno));

	snprintf(_carousel_root, sizeof(_carousel_root), "%s/%u", CAROUSELS_DIR, elementary_pid);
	if(mkdir(_carousel_root, 0755) < 0 && errno != EEXIST)
		fatal("Unable to create carousel directory '%s': %s", _carousel_root, strerror(errno));

	snprintf(_carousel_root, sizeof(_carousel_root), "%s/%u/%u", CAROUSELS_DIR, elementary_pid, carousel_id);
	if(mkdir(_carousel_root, 0755) < 0 && errno != EEXIST)
		fatal("Unable to create carousel directory '%s': %s", _carousel_root, strerror(errno));
...
save_file(char *kind, uint16_t elementary_pid, uint32_t carousel_id, uint16_t module_id, char *key, uint32_t key_size, unsigned char *file, uint32_t file_size)
{
	...
	root = make_carousel_root(elementary_pid, carousel_id);
...
void
make_service_root(uint16_t service_id, char *kind, uint16_t elementary_pid, uint32_t carousel_id, uint16_t module_id, char *key, uint32_t key_size)
{
	...
	snprintf(dirname, sizeof(dirname), "%s", SERVICES_DIR);
	...
	snprintf(realfile, sizeof(realfile), "../%s/%u/%u/%s-%u-%s", CAROUSELS_DIR, elementary_pid, carousel_id, kind, module_id, ascii_key);
...
char *
make_dir(char *kind, uint16_t elementary_pid, uint32_t carousel_id, uint16_t module_id, char *key, uint32_t key_size)
{
	char *root;
	char *ascii_key;
...
void
add_dir_entry(char *dir, char *entry, uint32_t entry_size, char *kind, uint16_t elementary_pid, uint32_t carousel_id, uint16_t module_id, char *key, uint32_t key_size)
{
	char *ascii_key;
	char realfile[PATH_MAX];
	char linkfile[PATH_MAX];
